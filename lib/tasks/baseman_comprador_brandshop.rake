...
namespace :baseman_comprador do

  desc "TODO"
  task sync: :environment do
    bm = Benchmark.measure do

      campaign_id = Rails.application.config.baseman_comprador_brandshop[:campaign_id]
      resource = RestClient::Resource.new("#{Rails.application.config.baseman_comprador_brandshop[:base_url]}/api/v1/baseman.json", headers: { charset: "utf-8", authorization: "#{ActionController::HttpAuthentication::Token.encode_credentials(Rails.application.config.baseman_comprador_brandshop[:token])}" })
      puts campaign_id.inspect
      loop do
